...
cov_df = read_csv("./Results/sensitivity_analysis/cov_df.csv")
...
metabolites_mi = MI_all_results(metabolites_outcomes, gee_df = gee_df_metabolites, out.dir = "./Results/sensitivity_analysis/Before_MC", name = "brain_metabolites")
...
CV_mi = MI_all_results(CV_outcomes, gee_df = gee_df_CV, out.dir = "./Results/sensitivity_analysis/Before_MC", name = "cortical_volume", outlier_ind = outlier_ind_CV, oc = "brain_region")
...
CT_mi = MI_all_results(CT_outcomes, gee_df = gee_df_CT, out.dir = "./Results/sensitivity_analysis/Before_MC", name = "cortical_thickness", outlier_ind = outlier_ind_CT, oc = "brain_region")
...
Sub_mi = MI_all_results(Sub_outcomes, gee_df = gee_df_Sub, out.dir = "./Results/sensitivity_analysis/Before_MC", name = "subcortical_roi", outlier_ind = outlier_ind_Sub, oc = "brain_region")
...
Neuro_mi = MI_all_results(Neuro_outcomes, gee_df = gee_df_neurobh, out.dir = "./Results/sensitivity_analysis/Before_MC", name = "neurobehavior", outlier_ind = outlier_ind_nb, glm_ind = glm_ind_nb)
...
metabolites_qg = qg_all_results(metabolites_df_name, metabolites_outcomes, out_type = "metabolites", oc = "other", out.dir = "./Results/sensitivity_analysis/Before_MC", name = "brain_metabolites") %>% mutate(type = "metabolites")
CV_qg = qg_all_results(CV_df_name, CV_outcomes, out_type = "brain_region", oc = "brain_region", out.dir = "./Results/sensitivity_analysis/Before_MC", name = "cortical_volume") %>% mutate(type = "CV")
CT_qg = qg_all_results(CT_df_name, CT_outcomes, out_type = "brain_region", oc = "brain_region", out.dir = "./Results/sensitivity_analysis/Before_MC", name = "cortical_thickness") %>% mutate(type = "CT")
Sub_qg = qg_all_results(Sub_df_name, Sub_outcomes, out_type = "brain_region", oc = "brain_region", out.dir = "./Results/sensitivity_analysis/Before_MC", name = "subcortical_roi") %>% mutate(type = "Sub")
Neuro_qg = qg_all_results(neurobh_df_name, Neuro_outcomes, out_type = "neurobehavior", glm_ind = glm_ind_nb, oc = "other", out.dir = "./Results/sensitivity_analysis/Before_MC", name = "neurobehavior") %>% mutate(type = "Neuro")
...
write_csv(agreed_windows, "./Results/sensitivity_analysis/model_agreement/agreed_results.csv")

# Create Brain Function Map
#agreed_windows = read_csv("./Results/sensitivity_analysis/model_agreement/agreed_results.csv")
...
#func = c("Cognition", "Movement", "Language", "Movement", "Cognition", "Auditory", "Cognition", "Cognition")
...
#brain_df %>% filter(anatomy == "Frontal Lobe") %>% dplyr::select("outcome", "window", "estimate", "type")
#brain_df %>% filter(as.numeric(estimate) < 0) %>% dplyr::select("outcome", "window", "estimate", "type", "anatomy")

# Mediation
sim_files = list.files("./Results/sensitivity_analysis/simulation", recursive = TRUE, full.names = TRUE) 
sim_result = lapply(sim_files, function(x) read_csv(x)) %>% bind_rows() %>% na.omit(sig) %>% add_column(type = c("CV", "CV", "Sub"), .after = "mediator") %>% 
  add_column(model = c("mi", "qg", "mi"), .after = "sig") %>% mutate(OPE = case_when(is.na(OPE) ~ "OPE",
                                                                                           .default = toupper(OPE)),
                                                                           ind = c(3, 3, 5)) 
...
write_csv(sim_result, "./Results/sensitivity_analysis/mediation/mediation_result.csv")
